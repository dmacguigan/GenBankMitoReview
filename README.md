...
The [`examples/`](examples/) directory holds a complete run on RefSeq sea star
(Asteroidea) mitogenomes, produced with:

```bash
bash mitoreview.sh --refseq --outdir examples '"Asteroidea"[Organism]' asteroidea
```

The figures below come from that run (27 genomes). The raw `.gb` downloads are
omitted from the directory since they are regenerated by the command above; the
tables, [summary](examples/asteroidea.summary.txt), and
[report](examples/asteroidea.report.md) are kept.

Start codon frequency by gene (red border = non-canonical for the genetic code):

![Start codon heatmap](examples/asteroidea.fig_start_heatmap.png)

Stop codon frequency by gene:

![Stop codon heatmap](examples/asteroidea.fig_stop_heatmap.png)

Start and stop codon distributions as stacked bars (hatched = non-canonical):

![Stacked codon bars](examples/asteroidea.fig_stacked_bars.png)

CDS length distribution by gene:

![CDS length ridgeline](examples/asteroidea.fig_geneLen_ridgeline.png)

tRNA, rRNA, and control region counts per genome:

![Non-coding feature counts](examples/asteroidea.fig_noncoding_counts.png)

tRNA length distributions by gene:

![tRNA length ridgeline](examples/asteroidea.fig_trna_lengths.png)

rRNA and control region length distributions:

![rRNA and control region lengths](examples/asteroidea.fig_rrna_cr_lengths.png)
